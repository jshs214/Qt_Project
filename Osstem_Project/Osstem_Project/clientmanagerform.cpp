    ...
    ui(new Ui::ClientManagerForm)
{
    ...
    connect(ui->clientTreeWidget, SIGNAL(customContextMenuRequested(QPoint)), this,
...
void ClientManagerForm::on_searchPushButton_clicked()
{
    ...
    {   /* 검색과 일치하거나 포함하는 문자열이 있으면 hidden(false) */
        auto items = ui->clientTreeWidget->findItems(ui->searchLineEdit->text(),
                                                     Qt::MatchContains, i);
...
void ClientManagerForm::receiveClientName(QString str)
{
    ...
    auto id = ui->clientTreeWidget->findItems(str, flag,0);        //키값비교, 0
    foreach(auto i, id) {
    ...
    auto items = ui->clientTreeWidget->findItems(str, flag,1);     //이름이므로 1
    foreach(auto i, items) {
        ClientItem* c = static_cast<ClientItem*>(i);
        int id = c->id();
        QString name = c->getName();
