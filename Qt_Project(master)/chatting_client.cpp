...
Chatting_Client::Chatting_Client(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Chatting_Client), isSent(false)
{
    ui->setupUi(this);
...
//프로토콜 생성해서 서버로 전송
void Chatting_Client::sendProtocol(Chat_Status type, char* data, int size)
{
...
// 파일 전송 시 여러번 나눠서 전송
void Chatting_Client::goOnSend(qint64 numBytes) // Start sending file content
{
...
//파일 보내기
void Chatting_Client::sendFile() // Open the file and get the file name (including path)
{
    loadSize = 0;
    byteToWrite = 0;
    totalSize = 0;
...
void Chatting_Client::on_logoutButton_clicked()
{
